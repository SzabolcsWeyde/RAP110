...
CLASS lhe_travel IMPLEMENTATION.
  ...
  METHOD on_travel_accepted.
    ...
    DATA lr_travel_accepted TYPE zrap110_etrav022.

    cl_abap_tx=>save( ).
    " loop over transferred travel instances and do the needful :)
    LOOP AT accepted REFERENCE INTO DATA(lr_accepted).
      ...
      " insert to db
      INSERT zrap110_etrav022 FROM @lr_travel_accepted.
    ENDLOOP.
  ...
  METHOD on_travel_rejected.
    ...
    DATA lr_travel_rejected TYPE zrap110_etrav022.
    ...
    LOOP AT rejected REFERENCE INTO DATA(lr_rejected).
      ...
      " insert to db
      INSERT zrap110_etrav022 FROM @lr_travel_rejected.
    ENDLOOP.
  ENDMETHOD.
